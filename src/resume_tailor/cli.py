...
@main.command()
@click.argument("markdown_resume_path", type=click.Path(exists=True, path_type=Path))
@click.option(
    "--output-path",
    type=click.Path(path_type=Path),
    default=None,
    help="DOCX path to write. Defaults to the Markdown file name with .docx.",
)
def export_docx(markdown_resume_path: Path, output_path: Path | None) -> None:
    """Export a reviewed Markdown resume to an ATS-safe DOCX."""
    markdown_text = markdown_resume_path.read_text(encoding="utf-8")
    docx_path = output_path or markdown_resume_path.with_suffix(".docx")
    summary_path = docx_path.with_name(f"{docx_path.stem}_export_summary.md")

    document = _markdown_resume_to_docx(markdown_text)
    document.save(docx_path)
    summary_path.write_text(
        _render_export_summary(markdown_resume_path, docx_path),
        encoding="utf-8",
    )
...
def _markdown_resume_to_docx(markdown_text: str) -> Document:
    document = Document()
    _configure_ats_safe_document(document)

    pending_paragraph: list[str] = []
    for raw_line in markdown_text.splitlines():
        line = raw_line.strip()
        if not line:
            _flush_plain_paragraph(document, pending_paragraph)
            continue
...
def _configure_ats_safe_document(document: Document) -> None:
...
def _flush_plain_paragraph(document: Document, lines: list[str]) -> None:
    if not lines:
        return
    paragraph = document.add_paragraph()
    paragraph.add_run(" ".join(lines))
    lines.clear()


def _render_export_summary(markdown_resume_path: Path, docx_path: Path) -> str:
    return "\n".join(
        [
            f"# Export Summary: {docx_path.name}",
            "",
            f"- Source Markdown: `{markdown_resume_path}`",
            f"- DOCX Output: `{docx_path}`",
            "- Layout: single column",
            "- Headings: standard Word heading styles",
            "- Lists: standard Word bullet lists",
            "- ATS constraints: no tables, text boxes, icons, images, or complex graphics generated by exporter",
            "",
        ]
    )
